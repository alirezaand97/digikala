            ...
            <div class="table-responsive-md table-responsive-lg table-responsive-xl table-responsive-sm">
                <table class="table table-hover">
                    ...
                    <tr v-for="(item,key2) in selectedProducts" v-bind:key="key2">
                        <td>
                            <img v-bind:src="$siteUrl+'files/thumbnails/'+item.get_product.image_url"
    ...
    export default {
        ...
        methods: {
            getProductWarranty: function (page) {
                const url = this.$siteUrl + 'admin/stockrooms/input/get-products?page=' + page + '&search=' + this.searchText;
                this.axios.get(url).then(response => {
                    for (let i = 0; i < response.data.data.length; i++) {
                        this.productCount[i] = response.data.data[i].product_number;
                    }
                ...
                if (validation) {
                    ...
                    this.axios.post(url, formData).then(response => {
                        if (response.data === 'success') {
                            this.serverError = '';
                            window.location = this.$siteUrl + 'admin/stockrooms/input/events';
                        } else {
