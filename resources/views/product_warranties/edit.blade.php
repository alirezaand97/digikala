...
@section('content')
   ...
   'current'=>['title'=>'ویرایش تنوع قیمت']
   ])
    @include('includes.warning-message')

    <div class="pannel">
        <h3 class="admin_pages_title">ویرایش تنوع قیمت برای {{$product->title}}</h3>
        {!! Form::model($product_warranties,['url' => 'admin/product_warranties/'.$product_warranties->id.'?product_id='.$product->id, 'files' => 'true']) !!}
        {{method_field('PUT')}}
        <div class="row">
            ...
            <div class="col-sm-12">
                <button class="btn digi_btn">ویرایش اطلاعات</button>
            </div>
            {!! Form::close() !!}
        </div>
    </div>
...
@section('scripts')
    <?php
   // TODO: ویرگول های اعداد باعث خطا می شوند
    ?>
